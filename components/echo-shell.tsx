...
function Avatar() {
  return (
    <div className="grid size-9 place-items-center rounded-full bg-surface-3 text-xs font-semibold text-foreground ring-1 ring-border">
      {user.initials}
    </div>
  );
}
...
function Onboarding({
  ...
  onComplete: (expense: StoredExpense) => void;
}) {
  ...
  const startListening = () => {
    setMode("listening");
    timer.current = setTimeout(() => setMode("confirm"), 900);
  };
  const finishListening = () => {
    if (timer.current) clearTimeout(timer.current);
    if (mode === "listening") setMode("confirm");
  };
  const save = (amount: number, merchant: string) => {
    ...
    setStep("done");
    setMode(null);
    setTimeout(() => onComplete(expense), 700);
  };
  return (
    <div className="onboarding">
      ...
      {step === "welcome" && (
        <div className="onboarding-copy">
          ...
          <p>
            Echo remembers the little things you spend on, so you can ask better
            questions later.
          </p>
          <button className="primary-button" onClick={() => setStep("capture")}>
            Tell Echo one thing <ArrowUpRight size={18} />
          </button>
        </div>
      )}
      {step === "capture" && (
        <div className="onboarding-copy">
          ...
          <p>
            Say it naturally. Echo will remember the moment, not make you fill
            out a form.
          </p>
          {mode === "confirm" ? (
            <div className="confirm-card">
              ...
              <p className="mt-5 text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">
                  “Spent {confirmedAmount} on lunch.”
                </span>
              </p>
              <p className="mt-1 text-sm text-muted-foreground">
                Did you say {money(confirmedAmount)} or {money(115)}?
              </p>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button
                  className={`choice-button ${confirmedAmount === 180 ? "choice-selected" : ""}`}
                  onClick={() => setConfirmedAmount(180)}
                >
                  {money(180)}
                </button>
                <button
                  className={`choice-button ${confirmedAmount === 115 ? "choice-selected" : ""}`}
                  onClick={() => setConfirmedAmount(115)}
                >
                  {money(115)}
                </button>
              </div>
              <button
                ...
                onClick={() => save(confirmedAmount, "Lunch")}
              >
            ...
            <div className="onboarding-listening">
              ...
              <p className="text-sm text-muted-foreground">
                Keep holding, or let go when you&apos;re done.
              </p>
            </div>
          ) : (
            <>
              <button
                ...
                aria-label="Hold to tell Echo an expense"
                onPointerDown={startListening}
                onPointerUp={finishListening}
                onPointerLeave={finishListening}
                onClick={() => mode === null && startListening()}
              >
                <Mic size={34} />
          ...
          {mode === "manual" && (
            <ManualForm
              ...
              setManual={setManual}
              onSave={() =>
                save(Number(manual.amount), manual.merchant || "Expense")
              }
      ...
      {step === "done" && (
        <div className="onboarding-copy">
          <div className="welcome-glyph success">
            <Check size={26} />
          </div>
          <p className="eyebrow text-emerald">Remembered</p>
          <h1>Nice. Echo has your back.</h1>
          <p>
            That&apos;s your first financial event. Ask me anything about your
            money as your memory grows.
          </p>
        </div>
      )}
...
function ManualForm({
  ...
  manual: { amount: string; merchant: string; category: string };
  setManual: (value: {
    amount: string;
    merchant: string;
    category: string;
  ...
  return (
    <div className="manual-card">
      ...
      <button
        ...
        disabled={!manual.amount || !manual.merchant}
        onClick={onSave}
      >
        Remember expense <Check size={17} />
      </button>
    </div>
  );
}

function Header({ screen, onVoice }: { screen: Screen; onVoice: () => void }) {
  const titles = {
  ...
  return (
    <header className="flex items-center justify-between px-5 pb-4 pt-6">
      <div>
        {screen === "home" ? (
          <p className="text-sm text-muted-foreground">Tuesday, June 25</p>
        ) : (
          <p className="text-sm text-muted-foreground">Echo Plus</p>
        )}
...
function SummaryCard({ expenses }: { expenses: Transaction[] }) {
      ...
      <div className="mt-8 flex items-end justify-between">
        <div>
          ...
          <div className="mt-2 h-2 w-44 overflow-hidden rounded-full bg-surface-3">
            <div className="h-full w-[68%] rounded-full bg-emerald" />
          </div>
        </div>
...
function InsightCard({ item }: { item: (typeof insights)[number] }) {
  return (
    <article className={`insight-card ${item.tone}`}>
      ...
      <button className="mt-4 inline-flex items-center gap-1 text-sm font-medium">
...
function HomeScreen({
  ...
  return (
    <>
      <Header screen="home" onVoice={onVoice} />
      <main className="flex flex-col gap-5 px-5 pb-28">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button className="quick-action" onClick={onVoice}>
          ...
          >
            <span className="grid size-9 place-items-center rounded-xl bg-violet-soft text-violet">
              <Lightbulb size={17} />
            </span>
            <span>
        ...
        <div className="section-heading">
          <div>
            <p className="eyebrow">Memory coverage</p>
            <h2 className="mt-1 text-lg font-semibold">A little nudge</h2>
          </div>
          <button
        ...
        <div className="divide-y divide-border rounded-2xl bg-surface-1 px-4">
...
function ActivityScreen({
  ...
  const filters = ["All", "Groceries", "Entertainment", "Transport"];
  return (
    <>
      <Header screen="activity" onVoice={onVoice} />
      <main className="flex flex-col gap-5 px-5 pb-28">
  ...
  return (
    <section className="panel">
      <div className="flex items-center justify-between">
        ...
        <button className="icon-button" aria-label={`More about ${title}`}>
...
function InsightsScreen({ onVoice }: { onVoice: () => void }) {
  return (
    <>
      <Header screen="insights" onVoice={onVoice} />
      <main className="flex flex-col gap-5 px-5 pb-28">
...
function ProfileScreen({ onVoice }: { onVoice: () => void }) {
  const rows = [
    {
      icon: UserRound,
      label: "Personal details",
      note: "Name, email, preferences",
    },
    {
      icon: CreditCard,
      label: "Connected accounts",
      note: "2 accounts connected",
    },
  ...
  return (
    <>
      <Header screen="profile" onVoice={onVoice} />
      <main className="flex flex-col gap-5 px-5 pb-28">
        <div className="profile-card">
          ...
          <button className="ml-auto text-sm font-medium text-emerald">
            Edit
          </button>
        </div>
...
function BottomNav({
  ...
  return (
    <nav className="bottom-nav">
      ...
      <button
        className={screen === "activity" ? "nav-item active" : "nav-item"}
        onClick={() => setScreen("activity")}
      >
        <ListFilter size={19} />
        <span>Activity</span>
      ...
      <button
        className={screen === "insights" ? "nav-item active" : "nav-item"}
        onClick={() => setScreen("insights")}
      >
        <Lightbulb size={19} />
        <span>Insights</span>
...
function VoiceSheet({
  ...
  open: boolean;
  onClose: () => void;
  onSave: (expense: StoredExpense) => void;
}) {
  ...
  useEffect(() => {
    if (open) {
      setMode("listening");
      timer.current = setTimeout(() => setMode("confirm"), 850);
    }
    return () => {
      if (timer.current) clearTimeout(timer.current);
    };
  ...
  const save = (amount: number, merchant: string) => {
    onSave({
      id: `local-${Date.now()}`,
      merchant,
      category: manual.category || "Food & Drink",
      amount,
      date: "Today, just now",
      icon: merchant.charAt(0).toUpperCase(),
      tone: "orange",
  ...
  return (
    <div className="sheet-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        <div className="mx-auto mb-6 h-1 w-10 rounded-full bg-surface-3" />
        <div className="flex items-center justify-between">
          <div>
            <p className="eyebrow text-emerald">Quick capture</p>
            <h2 id="voice-title" className="mt-1 text-2xl font-semibold">
              {mode === "manual" ? "Add a memory" : "Tell Echo what happened"}
            </h2>
          </div>
          <button
            className="icon-button"
        ...
        {mode === "manual" ? (
          ...
          <div className="onboarding-listening">
            ...
            <p className="font-medium">Echo is listening</p>
            <p className="text-sm text-muted-foreground">
              Say something like “Spent 150 on lunch.”
            </p>
          </div>
        ) : (
          <div className="confirm-card mt-6">
            <div className="flex items-center gap-3">
              <div className="merchant-icon bg-orange-soft text-orange">L</div>
              <div>
                <p className="font-medium">Lunch</p>
                <p className="text-sm text-muted-foreground">
                  Food & Drink · Today
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Did you say {money(150)} or {money(115)}?
            </p>
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                className="choice-button choice-selected"
                onClick={() => save(150, "Lunch")}
              >
                {money(150)}
              </button>
              <button
                className="choice-button"
                onClick={() => save(115, "Lunch")}
              >
                {money(115)}
              </button>
            </div>
            <button
              ...
              onClick={() => setMode("manual")}
            >
              <Plus size={17} /> Add manually instead
            </button>
          </div>
        )}
      </section>
...
export function EchoApp() {
  ...
  const [expenses, setExpenses] = useState<Transaction[]>(seedTransactions);
  useEffect(() => {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    const done = window.localStorage.getItem(ONBOARDING_KEY);
    if (stored) setExpenses([...JSON.parse(stored), ...seedTransactions]);
    setOnboarded(done === "true");
    setReady(true);
  }, []);
  const persist = (expense: StoredExpense) => {
    setExpenses((current) => {
      const next = [expense, ...current];
      const localOnly = next.filter((item) => item.id.startsWith("local-"));
  ...
  if (!onboarded) return <Onboarding onComplete={persist} />;
  return (
    <div className="app-viewport">
      <div className="app-frame">
        <div className="app-content">
          ...
          {screen === "insights" && (
            <InsightsScreen onVoice={() => setVoiceOpen(true)} />
          )}
          {screen === "profile" && (
            <ProfileScreen onVoice={() => setVoiceOpen(true)} />
          )}
        </div>
        <BottomNav
          screen={screen}
